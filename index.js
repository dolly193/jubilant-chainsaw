    ...
    if (interaction.isChatInputCommand()) { // Lida com comandos de barra
        ...
        if (action === 'confirm-delete') {
            ...
            if (contextParts.length > 3) {
                context.productName = decodeURIComponent(contextParts.slice(3).join('_'));
            }
...
async function sendDeliveryNotification(deliveryData) {
    try {
        ...
        // Constrói o ID customizado com base no contexto
        const contextString = context.type === 'ticket'
            ? `${context.type}_${context.channelId}_${context.userId}_${context.productId}_${encodeURIComponent(context.productName)}`
            : `${context.type}_${context.orderId}_${context.userId}_${encodeURIComponent(context.productName)}`;
